...
def quantize_uniform_blocks(x: torch.Tensor, bits: int, alpha: float = 3.0) -> torch.Tensor:
    ...
    Each additional bit halves the step size (~6 dB/bit MSE reduction).

    v1 bug: scale=rms regardless of bits, so step size was constant
    and only clipping range changed. For b>=3, clipping never occurred,
    making 3b/4b/5b identical. Fix: scale = alpha*rms / half.

    Args:
        x: (..., 8) -- last dim is block_size=8.
        bits: quantization bitwidth (integer).
        alpha: RMS units covered by full grid range (3.0 = ±3σ, 99.7%).
    """
    n_levels = 2 ** bits
    half = n_levels / 2
